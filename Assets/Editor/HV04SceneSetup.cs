...
// Genera la escena HV04 completa desde cero:
// · Background hub04.png a escala (5,5,1) → sala 83.6u × 47.05u (≈3× HV01_Interior)
// · Cielo parallax con 4 estados de día/noche, trackPlayer=true (cámara estática)
// · Paredes, suelo, techo en layer 8 (Ground)
// · Player prefab, PlayerSpawnManager, SpawnPoint, DoorExit de retorno a HV02
// · CameraFollow FitRoom con roomBackground asignado → orthographicSize auto
public static class HV04SceneSetup
{
    // Dimensiones: hub04.png 1672×941px @ PPU=100 → 16.72×9.41u × escala 5 → 83.6×47.05u
    const float BG_SCALE   = 5f;
    const float SKY_SCALE  = 6f;   // más ancho que BG para cubrir parallax lateral
    ...
    [MenuItem("Eldoria/Setup HV04")]
    static void SetupHV04()
    {
        var scene = SceneManager.GetActiveScene();

        // ── Cargar assets ────────────────────────────────────────────────────
        var spBG        = Load<Sprite>("Assets/Sprites/Escenarios/Hub/hub04.png");
        ...
        // Player prefab por GUID conocido
        ...
        cam.orthographicSize = 23.5f;   // FitToRoom lo recalcula al detectar roomBackground
        ...
        camFollow.roomBackground = bgSR;   // FitToRoom ajustará orthographicSize en Start()

        // ── Cielo parallax ───────────────────────────────────────────────────
        // parallaxFactor=0.88 → cielo se mueve al 12% de la velocidad del player.
        // parallaxFactorY=1   → el cielo no sube/baja (estático verticalmente).
        // Con SKY_SCALE=6 el sprite mide 100.32u de ancho → cubre la cámara (83.6u)
        // incluso cuando el player está en el borde de la sala.
        ...
        var spawnGO    = Reg(new GameObject("SpawnPoint_DoorHV04"), "Spawn");
        ...
        so.FindProperty("spawnId").stringValue     = "door_HV04";
        ...
        {
            Debug.LogWarning("[HV04Setup] Prefab de Player no encontrado — añadir manualmente.");
        }

        EditorSceneManager.MarkSceneDirty(scene);
        Debug.Log("[HV04Setup] ¡Escena HV04 lista! Guarda con Ctrl+S.\n" +
    ...
    static T Load<T>(string path) where T : Object
    {
        ...
        if (asset == null) Debug.LogWarning($"[HV04Setup] Asset no encontrado: {path}");
        return asset;
    }
