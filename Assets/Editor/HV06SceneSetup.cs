...
// Genera la escena HV06 completa desde cero.
// Mismas dimensiones que HV04/HV05: hub06.png 1672×941px @ PPU=100 → escala 5 → 83.6×47.05u
public static class HV06SceneSetup
{
    ...
    [MenuItem("Eldoria/Setup HV06")]
    static void SetupHV06()
    {
        var scene = SceneManager.GetActiveScene();

        // ── Cargar assets ────────────────────────────────────────────────────
        var spBG        = Load<Sprite>("Assets/Sprites/Escenarios/Hub/hub06.png");
        ...
        var spawnGO    = Reg(new GameObject("SpawnPoint_DoorHV06"), "Spawn");
        ...
        so.FindProperty("spawnId").stringValue     = "door_HV06";
        ...
        {
            Debug.LogWarning("[HV06Setup] Prefab de Player no encontrado — añadir manualmente.");
        }

        EditorSceneManager.MarkSceneDirty(scene);
        Debug.Log("[HV06Setup] ¡Escena HV06 lista! Guarda con Ctrl+S.\n" +
    ...
    static SpriteRenderer SkyLayer(GameObject parent, string name, Sprite sprite, int order, float alpha)
    {
        ...
        px.parallaxFactor  = 0.95f;
        px.parallaxFactorY = 0.97f;
        px.trackPlayer     = true;
    ...
    static T Load<T>(string path) where T : Object
    {
        ...
        if (asset == null) Debug.LogWarning($"[HV06Setup] Asset no encontrado: {path}");
        return asset;
    }
